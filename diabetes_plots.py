...
def app(diabetes_df):
  ...
  if st.checkbox("Show the correlation heatmap"):
    ...
    ax = sns.heatmap(diabetes_df.iloc[:, 1:].corr(), annot = True)
  ...
  if plot_select == 'Decision Tree Classifier':
    ...
    cm = confusion_matrix(y_test, y_test_pred) # type: ignore
    ...
    if st.checkbox("Plot Decision Tree"):
      ...
      dot_data = tree.export_graphviz(decision_tree = dtree_clf, max_depth = 3,
  ...
  if plot_select == 'GridSearchCV Best Tree Classifier':
    ...
    if st.checkbox("Plot Decision Tree"):
      ...
      dot_data = tree.export_graphviz(decision_tree = best_tree, max_depth = 3,
                                      out_file = None, filled = True, rounded = True,
                                      feature_names = feature_columns, class_names = ['0', '1']
                                      )
      # Plot the decision tree using the 'graphviz_chart' function of the 'streamlit' module.
